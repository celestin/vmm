     ...
     if(!do_continue) begin
      ...
      begin
         ...
         if (req.kind == wb_cycle::WRITE ) begin
            ...
            if (dat !== req.data)
               `vmm_error(this.log, $psprintf("DAT_I not stable during write cycle: %h vs %h", dat, req.data));
            tag = this.sigs.sck.wtgd;
            if (tag !== req.tgd)
               `vmm_error(this.log, $psprintf("TGD_I not stable during write cycle: %h vs %h", tag, req.tgd));
         end

...
class wb_cycle extends vmm_data;
   ...
   constraint valid_address {
      if (cfg.port_size - cfg.granularity == 1) addr[0:0] == 0;
      if (cfg.port_size - cfg.granularity == 2) addr[1:0] == 0;
      if (cfg.port_size - cfg.granularity == 3) addr[2:0] == 0;
   }

   constraint valid_sel {
      sel inside {8'h01, 8'h03, 8'h07, 8'h0F, 8'h1F, 8'h3F, 8'h7F, 8'hFF};
      if (cfg.port_size - cfg.granularity == 0) sel[7:1] == 0;
      if (cfg.port_size - cfg.granularity == 1) sel[7:2] == 0;
      if (cfg.port_size - cfg.granularity == 2) sel[7:4] == 0;
   }

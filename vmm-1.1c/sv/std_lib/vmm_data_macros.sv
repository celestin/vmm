...
//	        sindextemp = string'(this.__vmm_maxbits); \
...
`define vmm_data_member_vmm_data(_name, _do, _how) \
  \
      case (do_what & _do) \
        DO_PRINT: begin \
          if ( _name == null ) begin \
              $sformat(this.__vmm_image, `" do-print 1jvmm_data _name object does not exist, no action`"); \
              `vmm_warning(this.log, this.__vmm_image); \
          end \
          else begin \
	     string _prefix = this.__vmm_prefix; \
        ...
        DO_COPY: begin \
           if (_name == null) begin \
             $sformat(this.__vmm_image,`" do-copy 1vmm_data  _name object does not exist, no action`"); \
             `vmm_warning(this.log, this.__vmm_image); \
           end \
           else begin \

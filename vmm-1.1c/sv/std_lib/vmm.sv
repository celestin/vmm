...
`ifndef VCS_VMM_VERSION_PATCH
`define VCS_VMM_VERSION_PATCH /**/
`endif
`endif
...
`ifndef VMM_SCENARIO_BASE_NEW_ARGS
   `define VMM_SCENARIO_BASE_NEW_ARGS        `VMM_DATA_BASE_NEW_ARGS
   `define VMM_SCENARIO_BASE_NEW_EXTERN_ARGS `VMM_DATA_BASE_NEW_EXTERN_ARGS
`endif
`ifndef VMM_SCENARIO_BASE_NEW_CALL
  `ifndef VMM_DATA_BASE_NEW_CALL
     `define VMM_SCENARIO_BASE_NEW_CALL /**/
  `else
     `define VMM_SCENARIO_BASE_NEW_CALL        `VMM_DATA_BASE_NEW_CALL
  `endif
`endif
`ifndef VMM_SCENARIO_BASE_METHODS
   `define VMM_SCENARIO_BASE_METHODS         `VMM_DATA_BASE_METHODS
`endif
...
`ifndef VMM_OVM_ENV_BASE
  `define VMM_OVM_ENV_BASE vmm_env
`endif
`ifndef VMM_ENV
...
class vmm_scenario extends `VMM_SCENARIO_BASE;
   ...
   extern function new(`VMM_LOG log `VMM_SCENARIO_NEW_ARGS);
...
class vmm_ms_scenario extends `VMM_SCENARIO;
    ...
    /*local*/ extern virtual function vmm_ms_scenario_gen Xget_context_genX();
...
// OVM_INTEROP - vmm_scenario_adapter
...
`ifndef VMM_GRAB_DISABLED
   ...
   /*local*/ extern protected task XputX(vmm_data obj, int offset = -1, `VMM_SCENARIO grabber = null);
...
`else
   ...
   /*local*/ extern protected task XputX(vmm_data obj, int offset = -1);
   extern task playback(output bit      success,
                        input  string   filename,
                        input  vmm_data factory,
                        input  bit      metered = 0);
`endif
   ...
   extern task get(output vmm_data obj,
                   input  int      offset = 0);
   // OVM_INTEROP: non-blocking get - make public
   extern /*local*/ function void get1(output vmm_data obj,
                                       input  int      offset = 0);
   extern task peek(output vmm_data obj,
                    input  int      offset = 0);
`ifdef OVM_INTEROP // non-blocking peek                    
...
`endif
   ...
   extern local function int index(int offset);
...
class vmm_xactor_iter;
  ...
  local static vmm_xactor _vmm_xactor;
  local int idx;

...
function vmm_data::new(vmm_log log
...
function string vmm_data::psdisplay(string prefix="");
   $sformat(psdisplay, "%0s#%0d.%0d.%0d", prefix,
...
function bit vmm_data::compare(       vmm_data to,
                               output string   diff,
                               input  int      kind = -1);
   string msg = "Virtual vmm_data::compare() not implemented in derivative";
   if (this.log == null) begin
     $display(msg);
     $finish;
   end
...
function int unsigned vmm_data::byte_size(int kind = -1);
   string msg = "Virtual vmm_data::byte_size() not implemented in derivative";
   if (this.log == null) begin
     $display(msg);
     $finish;
   end
...
function int unsigned vmm_data::byte_pack(ref   logic [7:0]  bytes[],
                                          ...
                                          input int          kind = -1);
   string msg = "Virtual vmm_data::byte_pack() not implemented in derivative";
   if (this.log == null) begin
      $display(msg);
      $finish;
   end
...
function int unsigned vmm_data::byte_unpack(const ref logic [7:0] bytes[],
                                            ...
                                            input int             len = -1,
                                            input int             kind = -1);
   string msg = "Virtual vmm_data::byte_unpack() not implemented in derivative";
   if (this.log == null) begin
      $display(msg);
      $finish;
   end
...
function bit vmm_data::load(int file);
   ...
   if (this.byte_unpack(bytes) != len) begin
      string msg = "Unable to fully unpack input record in vmm_data::load()";
      if (this.log == null) begin
         $display(msg);
         $finish;
      end

...
`ifndef VMM_GRAB_DISABLED
task vmm_channel::XputX(vmm_data obj,
                       int offset=-1,
                       `VMM_SCENARIO grabber=null);
`else
task vmm_channel::XputX(vmm_data obj,
...
function void vmm_channel::get1(output vmm_data obj,
                                 input  int      offset=0);
   if (offset == 0)
   begin
...
`endif
`ifdef OVM_INTEROP // non-blocking get
   this.unblock_producer();
`endif
   end // if obj != null
endfunction: get1
...
`ifndef VMM_GRAB_DISABLED
         this.XputX(factory.copy(), offset, grabber);
`else
         this.XputX(factory.copy(), offset);
`endif
     end
...
function int vmm_channel::index(int offset);
   ...
   begin
      ...
      begin
         $sformat(txt, "Invalid offset %0d specified. Not in {0:%0d}.\n",
                  offset, this.data.size()-1);
         void'(this.log.text(txt));
         this.log.end_msg();
      end
      index = -1;
...
function void vmm_channel::kill();
   ...
   if (!this.shared_log) this.log.kill();
endfunction: kill   

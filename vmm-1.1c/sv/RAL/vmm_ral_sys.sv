...
class vmm_ral_sys extends vmm_ral_block_or_sys;
   ...
   extern virtual function void power_down(bit retain = 0);
   extern virtual function void power_up(string power_domains = "");
...
function void vmm_ral_sys::power_down(bit retain = 0);
   vmm_ral_block blks[];
   string        domains[];
   ...
   this.get_blocks(blks, domains);
   foreach (blks[i]) begin
      blks[i].power_down(retain);
   end
endfunction: power_down


function void vmm_ral_sys::power_up(string power_domains = "");
   vmm_ral_block blks[];
   string        domains[];
   string        pwr_domain;
   ...
   foreach (blks[i]) begin
      ...
      blks[i].power_up(power_domains);
   end
endfunction: power_up

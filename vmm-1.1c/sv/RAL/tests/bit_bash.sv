...
begin
   ...
   if ($cast(blk, ral_model)) begin
      ...
      if (blk.get_attribute("NO_RAL_TESTS") == "" &&
          blk.get_attribute("NO_BIT_BASH_TEST") == "") begin

         _vmm_ral_tests.bit_bash(blk, "", log);
      end
   end
   else begin
      vmm_ral_sys sys;
      ...
      foreach (blks[i]) begin
         ...
         if (blks[i].get_attribute("NO_RAL_TESTS") != "" ||
             blks[i].get_attribute("NO_BIT_BASH_TEST") != "") continue;
         
         _vmm_ral_tests.bit_bash(blks[i], domains[i], log);
      end
   end

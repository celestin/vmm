...
virtual class vmm_sql_db;
   ...
   extern function vmm_sql_table create_table(string tablename,
                                              string scheme,
                                              byte datakind=255);
   /*extern*/ pure virtual function int status();
   /*extern*/ pure virtual function int statement(string sql_stmt);
   extern virtual function void commit();
   /*extern*/ pure virtual function void close();
...
function vmm_sql_table vmm_sql_db::Xcreate_table_baseX(string tablename,
                                                ...
                                                byte datakind=255,
                                                bit add_to_tables = 1); 
   vmm_sql_table sql_table = new(this.Xexpand_nameX(tablename), scheme,
                             this, datakind);
   if (this.statement(sql_table.Xget_schemaX())) begin
      sql_table = null;
   end
   else begin
      this.tables.push_back(sql_table);
   end
   ...
   if (add_to_tables && this.tables_table != null) begin
      void'(this.tables_table.insert($psprintf("\"%0s\", %0d", tablename,
                                         $unsigned(datakind))));
   end
   return (sql_table);

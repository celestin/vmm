...
`endif
   ...
   if (is_pattern[0]) begin
      ...
      this.is_path_pattern[0] = `vmm_str_match(this.pattern[0], "^@(.*)");
      if (is_path_pattern[0]) begin
         this.pattern[0] = `vmm_str_backref(this.pattern[0], 0);
      end
   end
   ...
   if (is_pattern[1]) begin
      ...
      this.is_path_pattern[1] = `vmm_str_match(this.pattern[1], "^@(.*)");
      if (is_path_pattern[1]) begin
         this.pattern[1] = `vmm_str_backref(this.pattern[1], 0);
      end
   end 
endfunction: reset
...
function vmm_log vmm_log::for_each();
   ...
   while (this.known_idx < this.known.size()) begin
	  ...
	  else if(is_path_pattern[0]) name_ok = vmm_path_match::match(that.name, this.pattern[0]);
      else                        name_ok = (that.name == this.pattern[0]);

      if (is_pattern[1])          inst_ok = `vmm_str_match(that.inst, this.pattern[1]);
	  else if(is_path_pattern[1]) inst_ok = vmm_path_match::match(that.inst, this.pattern[1]);
...
function bit vmm_log::name_and_inst_exists(string name,
                                           string inst);
   if (!(name == "" && inst == "") && 
       !(name == "/./" && inst == "/./")) begin
      bit matching_log_interface_found = 0;
      int idx = 0;

      this.reset(name, inst);
      while (idx < this.known.size()) begin
         vmm_log that = this.known[idx++];
         bit name_ok;
         bit inst_ok;
         ...
         if (name_ok && inst_ok) begin
            matching_log_interface_found = 1;
            break;
         end
      end
      if (matching_log_interface_found) begin
         return 1;
      end
      else begin
         return 0;
...
function void vmm_log::report(string name="/./",
   ...
   if (! this.name_and_inst_exists(name, inst)) begin
      `vmm_error(this, `vmm_sformatf("Couldn't find any message service interface with the specified name(instance) '%s(%s)'", name, inst));
      return;
   end
...
function void vmm_log::reset_message_count(int    severity = ALL_SEVS,
                                           string name = "",
                                           string inst = "",
                                           bit    recurse = 0);
   this.reset(name, inst, recurse);
   for(vmm_log log = this.for_each(); log != null; log = this.for_each()) begin
      ...
      if (severity & FATAL_SEV)   log.n_msg[FATAL_SEV]   = 0;
      if (severity & ERROR_SEV)   begin
         log.n_msg[ERROR_SEV]   = 0;
         this.error_count = 0;
      end
      if (severity & WARNING_SEV) log.n_msg[WARNING_SEV] = 0;
      if (severity & NORMAL_SEV)  log.n_msg[NORMAL_SEV]  = 0;
      if (severity & TRACE_SEV)   log.n_msg[TRACE_SEV]   = 0;
      if (severity & DEBUG_SEV)   log.n_msg[DEBUG_SEV]   = 0;
      if (severity & VERBOSE_SEV) log.n_msg[VERBOSE_SEV] = 0;
      if (severity & HIDDEN_SEV)  log.n_msg[HIDDEN_SEV]  = 0;
      if (severity & IGNORE_SEV)  log.n_msg[IGNORE_SEV]  = 0;
   end
endfunction: reset_message_count

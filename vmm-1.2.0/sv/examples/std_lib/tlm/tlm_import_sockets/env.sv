...
////////////////////////////////////////////////////////////
class my_vmm_tlm;
   typedef enum {TLM_REFUSED, TLM_WAIT, TLM_ACCEPTED, TLM_UPDATED, TLM_COMPLETED } sync_e;
endclass

class initiator extends vmm_xactor;
  my_trans trans = new();
  vmm_tlm_simple_initiator_socket#(initiator, my_trans, my_vmm_tlm) socket = new(this, "initiator_put");

  function new(vmm_xactor parent, string name);
    super.new(name, "inst1"  , 0, parent);
  endfunction
   
   virtual function vmm_tlm::sync_e nb_transport_bw(int id = -1, my_trans trans, my_vmm_tlm ph, int delay);
   endfunction 
...
class initiator_parent extends vmm_xactor;
   ...
   vmm_tlm_simple_initiator_socket#(initiator_parent, my_trans, my_vmm_tlm) socket = new(this, "initiator_P_put");
...
class target extends vmm_xactor;
   vmm_tlm_simple_target_socket#(target, my_trans, my_vmm_tlm) socket = new(this, "target_put");
   ...
   virtual function vmm_tlm::sync_e nb_transport_fw(int id=-1, my_trans trans, my_vmm_tlm ph, int delay);
   ...
   virtual task b_transport(int id= -1, my_trans trans , my_vmm_tlm ph = null, int delay = - 1);
   endtask : b_transport   
endclass : target

class target_parent extends vmm_xactor;
   target target_c;
   vmm_tlm_simple_target_socket#(target_parent, my_trans, my_vmm_tlm) socket = new(this, "target_P_put");
   ...
   virtual task b_transport(int id = -1, my_trans trans , my_vmm_tlm ph, int delay);
   endtask : b_transport

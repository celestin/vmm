...
class initiator extends vmm_xactor;
  my_trans trans = new();
  vmm_tlm_simple_initiator_socket#(initiator, my_trans, vmm_tlm) socket = new(this, "initiator_put");

  function new(vmm_object parent, string name);
    super.new(name , "inst1"  , 0, parent);
  endfunction
   
   virtual function vmm_tlm::sync_e nb_transport_bw(int id = -1, my_trans trans, vmm_tlm ph, int delay);
   endfunction 
...
class target extends vmm_xactor;
   vmm_tlm_simple_target_socket#(target, my_trans, vmm_tlm) socket = new(this, "target_put");
   ...
   virtual task b_transport(int id = -1, my_trans trans, vmm_tlm ph = null , int delay = -1 );
   endtask : b_transport   
endclass : target

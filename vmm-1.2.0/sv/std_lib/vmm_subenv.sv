...
task vmm_subenv::reset(vmm_env::restart_e kind = vmm_env::FIRM);
   this.state = STOPPED;
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[reset]");
`endif    
endtask: reset
...
task vmm_subenv::cleanup();
   ...
   this.state = CLEANED;
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[clean up]");
`endif   
endtask: cleanup
...
`ifdef VMM_TBLOG
function void vmm_subenv::tblog_step(vmm_debug::verbosity_e verbosity, string step_name);

...
`ifdef VMM_TBLOG
   tblog_state(vmm_debug::TRACE_SEV, "[idle]");
`endif   
...
`ifdef VMM_TBLOG
               tblog_state(vmm_debug::TRACE_SEV, "[busy]");
`endif               
...
`ifdef VMM_TBLOG
   tblog_state(vmm_debug::TRACE_SEV, "[request to start]");
`endif
   this.start_it = 1;
   this.stop_it  = 0;
   -> this.control_event;
...
`ifdef VMM_TBLOG
   tblog_state(vmm_debug::TRACE_SEV, "[request to stop]");
`endif
   this.start_it = 0;
   this.stop_it  = 1;
   -> this.control_event;
...
`ifdef VMM_TBLOG
   if (rst_typ == SOFT_RST) tblog_state(vmm_debug::TRACE_SEV, "[request to soft reset]");
   else if (rst_typ == FIRM_RST) tblog_state(vmm_debug::TRACE_SEV, "[request to firm reset]");
   else if (rst_typ == HARD_RST) tblog_state(vmm_debug::TRACE_SEV, "[request to hard reset]");
`endif
   this.start_it = 0;
   this.reset_it = 1;
   -> this.control_event;
...
`ifdef VMM_TBLOG
function void vmm_xactor::tblog_state(vmm_debug::verbosity_e verbosity, string state_name);
   if (verbosity == vmm_debug::NORMAL_SEV) `vmm_vpd_str_normal(state_name)
   else if (verbosity == vmm_debug::TRACE_SEV) `vmm_vpd_str_trace(state_name)
   else if (verbosity == vmm_debug::DEBUG_SEV) `vmm_vpd_str_debug(state_name)
   else if (verbosity == vmm_debug::VERBOSE_SEV) `vmm_vpd_str_verbose(state_name)
endfunction
`endif

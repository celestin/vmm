...
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[pre test]");
`endif 
...
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[generate configuration]");
`endif    
   if (this.log.start_msg(vmm_log::INTERNAL_TYP , vmm_log::TRACE_SEV )) begin
...
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[build]");
`endif    
   if (this.log.start_msg(vmm_log::INTERNAL_TYP , vmm_log::TRACE_SEV )) begin
...
`ifdef VMM_12_UNDERPIN_VMM_ENV
   if (this.get_parent_object()!=null) begin
      `vmm_warning(this.log, "vmm_env instance should be singleton root object , hence removing this child instance from implicit phasing");
      this.set_parent_object(null);
   end
   else this.run_phase("build");
`endif
...
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[reset dut]");
`endif    
   if (this.log.start_msg(vmm_log::INTERNAL_TYP , vmm_log::TRACE_SEV )) begin
...
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[configure dut]");
`endif    
   if (this.log.start_msg(vmm_log::INTERNAL_TYP , vmm_log::TRACE_SEV )) begin
...
`ifdef VMM_12_UNDERPIN_VMM_ENV
   ...
   else this.run_phase("config_dut");
`endif
endtask: cfg_dut
...
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[start]");
`endif    
   if (this.log.start_msg(vmm_log::INTERNAL_TYP , vmm_log::TRACE_SEV )) begin
...
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[wait for end]");
`endif    
   if (this.log.start_msg(vmm_log::INTERNAL_TYP , vmm_log::TRACE_SEV )) begin
...
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[stop]");
`endif    
   if (this.log.start_msg(vmm_log::INTERNAL_TYP , vmm_log::TRACE_SEV )) begin
...
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[cleanup]");
`endif    
   if (this.log.start_msg(vmm_log::INTERNAL_TYP , vmm_log::TRACE_SEV )) begin
...
`ifdef VMM_TBLOG
   if (reconfig) tblog_step(vmm_debug::TRACE_SEV, "[hard restart]");
   else tblog_step(vmm_debug::TRACE_SEV, "[soft restart]");
`endif 
...
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[restart test]");
`endif 
...
`ifdef VMM_12_UNDERPIN_VMM_ENV
   ...
   else this.run_phase("report");
`endif
   this.log.report("/./", "/./");
   this.notify.indicate(REPORT);
`ifdef VMM_TBLOG
   tblog_step(vmm_debug::TRACE_SEV, "[report]");
`endif   
endtask: report

`ifdef VMM_TBLOG
function void vmm_env::tblog_step(vmm_debug::verbosity_e verbosity, string step_name);
   if (verbosity == vmm_debug::NORMAL_SEV) `vmm_vpd_str_normal(step_name)
   else if (verbosity == vmm_debug::TRACE_SEV) `vmm_vpd_str_trace(step_name)
   else if (verbosity == vmm_debug::DEBUG_SEV) `vmm_vpd_str_debug(step_name)
   else if (verbosity == vmm_debug::VERBOSE_SEV) `vmm_vpd_str_verbose(step_name)
endfunction
`endif

...
`ifdef VMM_12_UNDERPIN_VMM_XACTOR
         ...
         if (obj == null) `vmm_trace(log,`vmm_sformatf("the scenario '%s' doesn't exist at hierarchy %s",sname,ghier));
            ...
            if (!$cast(gen, obj) && !$cast(gen, obj)) return;
...
`ifdef VMM_12_UNDERPIN_VMM_XACTOR
            obj = vmm_object::find_object_by_name(ghier); //checking if atomic gen exists
            if (obj != null && ($cast(sgen, obj))) begin
                set_scn_items(sname, sgen, str_idx, tr);
            end
            return;
`else
            split_hiername(ghier, gname, hier);
...
`ifdef VMM_12_UNDERPIN_VMM_XACTOR
            obj = vmm_object::find_object_by_name(ghier); //checking if atomic gen exists
            if (obj != null && ($cast(agen, obj))) begin
               if (fact_type == CREATE_COPY)  agen.Xset_blueprintX(tr.copy()); 
               else agen.Xset_blueprintX(tr.allocate());
            end
            return;
`else
            split_hiername(ghier, gname, hier);
...
class vmm_class_factory_base#(type classname=vmm_object);
   ...
   static function classname Xfactory_typeX();
      if (XctypeX == null) XctypeX = new();
      return XctypeX;
   endfunction
...
`define vmm_class_factory(classname) \
 ...
 \
  static function classname this_type(); \
     return _factory_base::Xfactory_typeX(); \
  endfunction \
 \
  static function classname create_instance(vmm_object parent, \
                                            string name, \

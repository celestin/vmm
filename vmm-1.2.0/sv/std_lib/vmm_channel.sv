...
`ifdef VMM_TBLOG
   tblog_trans(vmm_debug::TRACE_SEV, "[put]\n", obj);
`endif
...
`ifdef VMM_TBLOG
   tblog_trans(vmm_debug::TRACE_SEV, "[sneak]\n", obj);
`endif
...
task vmm_channel::get(output vmm_data obj,
   ...
   this.unblock_producer();
`ifdef VMM_TBLOG
   tblog_trans(vmm_debug::TRACE_SEV, "[get]\n", obj);
`endif   
endtask: get
...
`ifdef VMM_TBLOG
   tblog_trans(vmm_debug::TRACE_SEV, "[peek]\n", obj);
`endif   
endtask: peek
...
`ifdef VMM_TBLOG
   tblog_trans(vmm_debug::TRACE_SEV, "[activate]\n", obj);
`endif
...
`ifdef VMM_TBLOG
   tblog_trans(vmm_debug::TRACE_SEV, "[tee]\n", obj);
`endif   
endtask: tee
...
function void vmm_channel::connect(vmm_channel downstream);
   ...
   if ( this.downstream == downstream )
		begin
			$sformat(txt, "Attempt to connect two channels %s(%s) and %s(%s) which are already connected together", this.log.get_name(), this.log.get_instance(), downstream.log.get_name(), downstream.log.get_instance());
			`vmm_note(this.log, txt);
			return;
		end
	...
	if (this.downstream != null)
		begin
			$sformat(txt, "Attempt to connect channel %s(%s) which is already connected", this.log.get_name(), this.log.get_instance());
			`vmm_fatal(this.log, txt);
		end	

	if (downstream.downstream != null)
		begin
			$sformat(txt, "Attempt to connect channel %s(%s) which is already connected", downstream.log.get_name(), downstream.log.get_instance());
			`vmm_fatal(downstream.log, txt);
		end
...
function void vmm_channel::tblog_trans(vmm_debug::verbosity_e verbosity, string action, vmm_data data);
   string tblog_str;
   
   if (data == null) tblog_str = {action, "data unavailable"};
   else tblog_str = data.psdisplay(action);
   
   if (verbosity == vmm_debug::NORMAL_SEV) `vmm_vpd_str_normal(tblog_str)
   else if (verbosity == vmm_debug::TRACE_SEV) `vmm_vpd_str_trace(tblog_str)
   else if (verbosity == vmm_debug::DEBUG_SEV) `vmm_vpd_str_debug(tblog_str)
   else if (verbosity == vmm_debug::VERBOSE_SEV) `vmm_vpd_str_verbose(tblog_str)
endfunction

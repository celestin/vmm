...
`ifdef VMM_TBLOG
   tblog_event(vmm_debug::TRACE_SEV, `vmm_sformatf("[notification %0d is indicated]\n", notification_id), status);
`endif
...
`ifdef VMM_TBLOG
   if (rst_typ == HARD) tblog_event(vmm_debug::TRACE_SEV, `vmm_sformatf("[notification %0d is hard reset]\n", notification_id));
   else if (rst_typ == SOFT) tblog_event(vmm_debug::TRACE_SEV, `vmm_sformatf("[notification %0d is soft reset]\n", notification_id));
`endif   
endfunction: reset
...
`ifdef VMM_TBLOG
function void vmm_notify::tblog_event(vmm_debug::verbosity_e verbosity, string event_desc, vmm_data data = null);
   string tblog_str;
   
   tblog_str = event_desc;
   
   if (data == null) tblog_str = {event_desc, "no attached data"};
   else tblog_str = {event_desc, data.psdisplay("attached data is: \n")};

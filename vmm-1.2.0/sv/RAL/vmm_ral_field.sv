...
class vmm_ral_field extends vmm_object;
...
task vmm_ral_field::read(output vmm_rw::status_e             status,
   ...
   this.parent.read(status, reg_value, path, domain, data_id, scenario_id, stream_id, fname, lineno);
...
task vmm_ral_field::poke(output vmm_rw::status_e              status,
   ...
   this.parent.peek(status, tmp, data_id, scenario_id, stream_id, fname, lineno);
   ...
   tmp |= value << this.lsb;
   this.parent.poke(status, tmp, data_id, scenario_id, stream_id, fname, lineno);

...
class vmm_sb_ds_base;
   typedef enum {EITHER, INPUT, EXPECT} kind_e;
endclass


class vmm_sb_ds #(type INP = vmm_data , type EXP = INP) extends vmm_sb_ds_base;
   ...
   vmm_tlm_analysis_export_inp#(vmm_sb_ds#(INP,EXP),INP) inp_ap = new(this,"vmm_sb_ds input analysis export",99,0); 
   vmm_tlm_analysis_export_exp#(vmm_sb_ds#(INP,EXP),EXP) exp_ap = new(this,"vmm_sb_ds expect analysis export",99,0);
   ...
   function EXP expect_in_order(EXP pkt,
                                int      exp_stream_id = -1,
                                int      inp_stream_id = -1,
                                bit      silent = 0);
   ...
   function EXP expect_out_of_order(EXP pkt,
                                                    int      exp_stream_id = -1,
                                                    int      inp_stream_id = -1,
                                                    bit      silent = 0);
      vmm_sb_ds_exp_streams#(EXP) exp_streams;
...
class vmm_sb_ds_callbacks#(type INP=vmm_data, EXP=INP);
   ...
   virtual function void pre_insert(input vmm_sb_ds#(INP,EXP) sb,
                                    input EXP          pkt,
                                    //input vmm_sb_ds#(INP,EXP):: kind_e kind,
   ...
   virtual function void post_insert(vmm_sb_ds#(INP,EXP) sb,
                                     EXP  pkt,
                                     int       exp_stream_id,
                                     int       inp_stream_id);
   endfunction: post_insert

   virtual function void matched(input vmm_sb_ds#(INP,EXP) sb,
                                 input EXP  pkt,
   ...
   virtual function void mismatched(input vmm_sb_ds#(INP,EXP) sb,
                                    input EXP  pkt,
                                    input int       exp_stream_id,
                                    input int       inp_stream_id,
                                    ref   int       count);
   ...
   virtual function void dropped(input vmm_sb_ds#(INP,EXP) sb,
                                 input EXP  pkts[],
                                 input int       exp_stream_id,
                                 input int       inp_stream_id,
                                 ref   int       count);
   ...
   virtual function void not_found(input vmm_sb_ds#(INP,EXP) sb,
                                   input EXP  pkt,
                                   input int       exp_stream_id,
                                   input int       inp_stream_id,
                                   ref   int       count);
   ...
   virtual function void orphaned(input vmm_sb_ds#(INP,EXP) sb,
                                  input EXP  pkts[],
                                  input int       exp_stream_id,
                                  input int       inp_stream_id,
                                  ref   int       count);
...
class vmm_sb_ds_iter #(type INP=vmm_data , type EXP=INP);
   ...
   /*local*/ extern function new(vmm_sb_ds#(INP,EXP) sb,
                                 int       exp_stream_id,
                                 int       inp_stream_id);
...
function int vmm_sb_ds::inp_stream_id(INP          pkt);
   EXP exp_pkt[] ;
   inp_stream_id = 0;
...
function int vmm_sb_ds::exp_stream_id(EXP          pkt);
   vmm_data temp;
   if($cast(temp,pkt))
...
function bit vmm_sb_ds::inp_insert(INP          pkt,
                               int               exp_stream_id = -1,
                               int               inp_stream_id = -1);
   EXP exp_pkt[] ;
   inp_insert = 0;
...
function bit vmm_sb_ds::exp_insert(EXP          pkt,
                               int               exp_stream_id = -1,
                               int               inp_stream_id = -1);
   vmm_sb_ds_exp_streams#(EXP) exp_streams;
   vmm_sb_ds_pkt_stream#(EXP)  pkt_stream;
...
function bit vmm_sb_ds::insert(vmm_data          pkt,
   ...
   `vmm_callback(vmm_sb_ds_callbacks#(INP,EXP),
                 post_insert(this, tmp_pkt, exp_stream_id, inp_stream_id));
...
function bit vmm_sb_ds::inp_remove(INP           pkt,
                               int               exp_stream_id = -1,
                               int               inp_stream_id = -1);
   EXP exp_pkt[] ;
   inp_remove = 0;
...
function bit vmm_sb_ds::exp_remove(EXP          pkt,
                               int               exp_stream_id = -1,
                               int               inp_stream_id = -1);
...
function bit vmm_sb_ds::transform(input  INP in_pkt,
                                  output EXP out_pkts[]);
   EXP temp;
...
function bit vmm_sb_ds::match(EXP actual,
                              EXP expected);
   return this.quick_compare(actual, expected);
endfunction: match

function bit vmm_sb_ds::quick_compare(EXP actual,
                                      EXP expected);
   return 1;
endfunction: quick_compare

function bit vmm_sb_ds::compare(EXP actual,
                                EXP expected);
...
function bit vmm_sb_ds::expect_with_losses(input  EXP pkt,
                                           output EXP matched,
                                           output EXP lost[],
                                           input  int      exp_stream_id = -1,
                                           input  int      inp_stream_id = -1,
...
function void vmm_sb_ds::write_inp(int id=-1, INP trans);
   `vmm_error(this.log,"vmm_sb_ds class function write_inp is not extended.");
endfunction
   
function void vmm_sb_ds::write_exp(int id=-1, EXP trans);
...
function void vmm_sb_ds::prepend_callback(vmm_sb_ds_callbacks#(INP,EXP) cb);
   if (cb == null) begin
...
function void vmm_sb_ds::append_callback(vmm_sb_ds_callbacks#(INP,EXP) cb);
   if (cb == null) begin
...
function void vmm_sb_ds::unregister_callback(vmm_sb_ds_callbacks#(INP,EXP) cb);
   foreach(this.callbacks[i]) begin
      if (this.callbacks[i] == cb) begin
         // Unregister it

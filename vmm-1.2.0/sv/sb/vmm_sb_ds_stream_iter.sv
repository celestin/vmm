...
class vmm_sb_ds_stream_iter#(type INP=vmm_data, type EXP=INP);
   ...
   extern /*local*/ function new(vmm_sb_ds#(INP,EXP)            sb,
   ...
   function EXP delete();
      ...
      if (this.str.pkts.size() == 0) begin
         ...
         if (this.sb.get_n_pending() == 0) this.sb.notify.indicate(vmm_sb_ds#(INP,EXP)::EMPTY);
      end
...
function vmm_sb_ds_stream_iter::new(vmm_sb_ds#(INP,EXP)            sb,
                                    vmm_sb_ds_pkt_stream#(EXP) str,
                                    int                  exp_id,
                                    int                  inp_id);
   if (str == null) begin
...
function void vmm_sb_ds_stream_iter::prepend(EXP pkt);
  ...
  begin
   ...
   this.sb.notify.reset(vmm_sb_ds#(INP,EXP)::EMPTY);
endfunction: prepend

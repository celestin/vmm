...
class vmm_ral_sys extends vmm_ral_block_or_sys;
   ...
   extern function bit Xget_addr_granularityX();
...
function void vmm_ral_sys::Xlock_modelX();
   ...
   void'(this.Xget_addr_granularityX());
endfunction: Xlock_modelX

function bit vmm_ral_sys::Xget_addr_granularityX();
   bit addr_granularity = this.is_byte_level_addr_granularity(); 

   foreach (this.domains[i]) begin
      foreach (this.domains[i].blocks[j]) begin
         if (addr_granularity != this.domains[i].blocks[j].is_byte_level_addr_granularity())
            `vmm_fatal(this.log, $psprintf("Inconsistent address granularity found between %s and %s.",
                                           this.get_fullname(), 
                                           this.domains[i].blocks[j].get_fullname()));
      end
      foreach (this.domains[i].subsys[j]) begin
         if (addr_granularity != this.domains[i].subsys[j].Xget_addr_granularityX())
            `vmm_fatal(this.log, $psprintf("Inconsistent address granularity found between %s and %s.",
                                           this.get_fullname(), 
                                           this.domains[i].subsys[j].get_fullname()));
      end
   end
   Xget_addr_granularityX = addr_granularity;
endfunction: Xget_addr_granularityX
...
function string vmm_ral_sys::psdisplay_domain(string prefix = "",
                                              string domain = "");
   string image;
   string domains[];
   string blk_domains[];
   ...
   if (single_domain) begin
      ...
      foreach (blks[i]) begin
         string img;
         img = blks[i].psdisplay_domain({prefix, "   "}, blk_domains[i]);
         image = {image, "\n", img};
      end
      ...
      foreach (sys[i]) begin
         string img;
         img = sys[i].psdisplay_domain({prefix, "   "}, blk_domains[i]);
         image = {image, "\n", img};
      end
   end
   else begin
      ...
      foreach (domains[i]) begin
         ...
         foreach (blks[j]) begin
            img = blks[j].psdisplay_domain({prefix, "      "},
                                    blk_domains[j]);
            image = {image, "\n", img};
         end
         ...
         foreach (sys[j]) begin
            img = sys[j].psdisplay_domain({prefix, "      "},
                                   blk_domains[j]);
            image = {image, "\n", img};
         end
      end
...
task vmm_ral_sys::mirror(output vmm_rw::status_e status,
   ...
   foreach (this.domains[i]) begin
      foreach (this.domains[i].blocks[j]) begin
         this.domains[i].blocks[j].mirror(status, check, path, fname, lineno);
         if (status != vmm_rw::IS_OK && status != vmm_rw::HAS_X) return;
      end
      foreach (this.domains[i].subsys[j]) begin
         this.domains[i].subsys[j].mirror(status, check, path, fname, lineno);
         if (status != vmm_rw::IS_OK && status != vmm_rw::HAS_X) return;
      end
   end
endtask: mirror

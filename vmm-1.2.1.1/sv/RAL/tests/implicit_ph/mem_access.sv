   ...
   foreach (ral_test.rals[i]) begin
      ...
      if ($cast(blk, ral_test.rals[i])) begin
         ...
         if (blk.get_attribute("NO_RAL_TESTS") == "" &&
             blk.get_attribute("NO_MEM_ACCESS_TEST") == "") begin
            
            blk.reset();
            vmm_ral_tests::mem_access(blk, log);
         end
      end
      else begin
         vmm_ral_sys   sys;
         ...
         foreach (blks[j]) begin
            ...
            if (blks[j].get_attribute("NO_RAL_TESTS") != "" ||
                blks[i].get_attribute("NO_MEM_ACCESS_TEST") != "") continue;
         
            blks[j].reset();
            vmm_ral_tests::mem_access(blks[i], log);
         end
      end
   end

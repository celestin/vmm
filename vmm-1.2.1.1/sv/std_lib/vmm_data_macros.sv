 ...
 \
      case (do_what & _do) \
        ...
        DO_UNPACK: begin \
           ...
           bit [31:0] size = 0; \
           start = this.__vmm_offset; \
           `ifdef VMM_CHECK_ARRAY_SIZE_FOR_UNPACK \
           if (unpack.size() < this.__vmm_offset) \
              `vmm_warning(log,"There is not enough data in the dynamic array to completly fill the descriptor"); \
           `endif \
           `vmm_data_member_scalar_unpackint(unpack,size,start,0) \
           `vmm_data_member_update_offset(this.__vmm_offset,4) \
           this.__vmm_maxbits =0; \

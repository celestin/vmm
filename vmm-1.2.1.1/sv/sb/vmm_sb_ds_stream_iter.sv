...
class vmm_sb_ds_stream_iter#(type INP=vmm_data, type EXP=INP);

   local vmm_sb_ds_typed#(INP,EXP) sb;
   local int       exp_id;
   local int       inp_id;

   local vmm_sb_ds_pkt_stream#(EXP) str;
   local int                  idx;

   local bit is_valid;

   extern /*local*/ function new(vmm_sb_ds_typed#(INP,EXP)            sb,
                                 vmm_sb_ds_pkt_stream#(EXP) str,
                                 int                  exp_id,
                                 int                  inp_id);
   ...
   extern function bit find(EXP pkt);
   extern function void prepend(EXP pkt);
   extern function void append(EXP pkt);
   ...
   function EXP delete();
      ...
      if (this.str.pkts.size() == 0) begin
         ...
         if (this.sb.get_n_pending() == 0) this.sb.notify.indicate(vmm_sb_ds_typed#(INP,EXP)::EMPTY);
      end
   ...
   function vmm_sb_ds_stream_iter#(INP,EXP) copy();
   
      vmm_sb_ds_stream_iter#(INP,EXP) iter = new(this.sb, this.str,
...
function vmm_sb_ds_stream_iter::new(vmm_sb_ds_typed#(INP,EXP)            sb,
                                    vmm_sb_ds_pkt_stream#(EXP) str,
                                    int                  exp_id,
                                    int                  inp_id);
   if (str == null) begin
      vmm_sb_ds_exp_streams#(EXP) exp_strs = null;
...
function bit vmm_sb_ds_stream_iter::find(EXP pkt);
...
function void vmm_sb_ds_stream_iter::prepend(EXP pkt);
  ...
  begin
   ...
   this.sb.notify.reset(vmm_sb_ds_typed#(INP,EXP)::EMPTY);
endfunction: prepend


function void vmm_sb_ds_stream_iter::append(EXP pkt);
  if(pkt == null)
  begin

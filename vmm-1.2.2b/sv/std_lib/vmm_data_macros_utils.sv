...
`define vmm_data_member_scalar_unpack(__unpack,__maxbits,__count,__offset) \
 \
        if (__offset+__count <= __unpack.size()) begin \
           for (int i=0; i < __count; i++) begin \
           ...
           `ifdef VMM_CHECK_ARRAY_SIZE_FOR_UNPACK \
              `vmm_warning(log,"unpack: There is not enough data in the dynamic array to completly fill the descriptor"); \
           `endif \
        end \
...
`define vmm_data_member_scalar_unpackint(__unpack,__index,__st,__endian) \
 \
     if ( __st+3 <=__unpack.size()) begin\
        ...
        `ifdef VMM_CHECK_ARRAY_SIZE_FOR_UNPACK \
           `vmm_warning(log,"unpackint: There is not enough data in the dynamic array to completly fill the descriptor"); \
        `endif \
    end \

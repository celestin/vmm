...
function string vmm_ss_scenario::__default_name(); 
	...
	`else 
       return {"Undefined ", "$typename() not defined", " Scenario"}; 
	`endif 
...
function void vmm_scenario_gen::reset_xactor(vmm_xactor::reset_e rst_typ = vmm_xactor::SOFT_RST); 
...
`ifdef VCS
		 ...
		 `else //Simplifying the code to ensure non-VCS simulators can compile it 
		    _tmp_last_idx = this.select_scenario.last_selected.size() - 1 ;
			this.select_scenario.next_in_set = ((this.select_scenario.last_selected[_tmp_last_idx] + 1) % this.scenario_set.size());
		 `endif	

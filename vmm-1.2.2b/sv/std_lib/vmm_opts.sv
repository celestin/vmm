...
function vmm_opts_info vmm_opts::get_opts_by_name(string name, 
   ...
   oinfo.doc       = doc;
   if (arg_type == vmm_opts_info::INT_ARGS) begin
      string my_tmp_str = "";
      int idx = -1;
      string format_char;
      int slen = oinfo.sarg.len();
      for (int i=0; i< slen; i++) begin
                 `ifndef MACRO_CAST
                  format_char = `_TO_CAST_TO_STRING(oinfo.sarg.getc(i));
                  `else
                   $cast(format_char,oinfo.sarg.getc(i));
                  `endif
         if (format_char == "b" || format_char == "o" || format_char == "x" || format_char == "h" || format_char == "d") begin
            idx = i+1;
            break;
         end
      end
      if (idx != -1 && idx < slen ) begin
         my_tmp_str = oinfo.sarg.substr(idx, slen-1);
         case (format_char)
           "b"      : oinfo.val = my_tmp_str.atobin();
           "h", "x" : oinfo.val = my_tmp_str.atohex();
           "o"      : oinfo.val = my_tmp_str.atooct();
           default  : oinfo.val = my_tmp_str.atoi();
         endcase
      end
      else begin

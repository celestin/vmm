...
`define vmm_data_member_scalar_queue(_name, _do) \
...
`define vmm_data_member_string_queue(_name, _do) \
...
\
    case (do_what & _do) \
	   ...
	   this.__vmm_maxbits = 0; \
	   `vmm_data_member_scalar_unpack(unpack,this.__vmm_maxbits,count,this.__vmm_offset) \
	   $cast(this._name,this.__vmm_maxbits[0+:31]); \
           `vmm_data_member_update_offset(this.__vmm_offset,count) \
        end \
      endcase

`define vmm_data_member_enum_queue(_name, _do) \
     ...
     case (do_what & _do) \
        ...
        DO_UNPACK: begin \
           ...
           for (int j=0; j < size; j++) begin \
                ...
                `vmm_data_member_scalar_unpack(unpack,this.__vmm_maxbits,count,this.__vmm_offset) \
                `vmm_data_member_update_offset(this.__vmm_offset,count) \
                stemp = string'(this.__vmm_maxbits); \
                index = 0; \
...
`define vmm_data_member_handle_queue(_name, _do) \
...
`define vmm_data_member_vmm_data_queue(_name, _do, _how) \
...
`define vmm_data_member_rand_scalar(_name, _do) \
  \
      case (do_what & _do) \
        DO_COPY: begin \
           __vmm_rhs._name = this._name; \
           if (!this._name.rand_mode()) begin \
              __vmm_rhs._name.rand_mode(0); \
           end \
           else begin \
              __vmm_rhs._name.rand_mode(1); \
           end \
        end \
      endcase

`define vmm_data_member_rand_scalar_queue(_name, _do) \
    `vmm_data_member_rand_scalar_array(_name, _do)

`define vmm_data_member_rand_scalar_array(_name, _do) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
           bit flg = 0; \
	   __vmm_rhs._name = this._name;	\
           foreach (this._name[i]) begin \
              if (this._name[i].rand_mode() != this._name[0].rand_mode()) \
                 flg = 1'b1; \
              if (!this._name[i].rand_mode()) __vmm_rhs._name[i].rand_mode(0); \
              else __vmm_rhs._name[i].rand_mode(1); \
           end \
           if (!flg && !this._name[0].rand_mode()) \
              __vmm_rhs._name.rand_mode(0); \
        end \
      endcase


`define vmm_data_member_rand_scalar_da(_name, _do) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
           bit flg = 0; \
...
`define vmm_data_member_rand_scalar_aa_scalar(_name, _do) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
           bit flg = 0; \
...
`define vmm_data_member_rand_scalar_aa_string(_name, _do) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
	   __vmm_rhs._name.delete(); \
	   foreach(this._name[i]) begin \
	      __vmm_rhs._name[i] = this._name[i]; \
`ifndef VCS_NO_RANDMODE_ON_ARRAY_ELEMENT \
                if (!this._name[i].rand_mode()) __vmm_rhs._name[i].rand_mode(0); \
                   else __vmm_rhs._name[i].rand_mode(1); \
`else \
                if (this._name[i].rand_mode() != this._name[0].rand_mode()) \
                   `vmm_warning(log,`vmm_sformatf("Rand state copy of string indexed assoc array is not supported if individual variables have different states",_name)); \
`endif \
	   end \
`ifdef VCS_NO_RANDMODE_ON_ARRAY_ELEMENT \
                if (!this._name[0].rand_mode()) __vmm_rhs._name.rand_mode(0); \
                    else __vmm_rhs._name.rand_mode(1); \
`endif \
        end \
      endcase


`define vmm_data_member_rand_enum(_name, _do) \
\
     case (do_what & _do) \
       DO_COPY: begin \
          __vmm_rhs._name = this._name; \
          if (!this._name.rand_mode()) __vmm_rhs._name.rand_mode(0); \
             else __vmm_rhs._name.rand_mode(1); \
       end \
     endcase


`define vmm_data_member_rand_enum_queue(_name, _do) \
    `vmm_data_member_rand_enum_array(_name, _do)

`define vmm_data_member_rand_enum_array(_name, _do) \
\
     case (do_what & _do) \
        DO_COPY: begin \
               bit flg = 0; \
...
`define vmm_data_member_rand_enum_da(_name, _do) \
\
     case (do_what & _do) \
        DO_COPY: begin \
                bit flg = 0; \
...
`define vmm_data_member_rand_enum_aa_scalar(_name, _do) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
           bit flg = 0; \
...
`define vmm_data_member_rand_enum_aa_string(_name, _do) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
	   __vmm_rhs._name.delete(); \
...
`else \
                if (this._name[i].rand_mode() != this._name[0].rand_mode()) \
                   `vmm_warning(log,{"Rand state copy of string indexed assoc array ", `"_name `", "is not supported if individual variables have different states"}); \
`endif \
	   end \
`ifdef VCS_NO_RANDMODE_ON_ARRAY_ELEMENT \
                if (!this._name[0].rand_mode()) __vmm_rhs._name.rand_mode(0); \
...
`define vmm_data_member_rand_handle(_name, _do) \
  \
      case (do_what & _do) \
        DO_COPY: begin \
           if ( _name == null ) begin \
...
`define vmm_data_member_rand_handle_queue(_name, _do) \
    `vmm_data_member_rand_handle_array(_name, _do)

`define vmm_data_member_rand_handle_array(_name, _do) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
                bit flg = 0; \
		foreach (this._name[i]) begin \
        	   __vmm_rhs._name[i] = this._name[i]; \
                   if (this._name[i].rand_mode() != this._name[0].rand_mode()) \
                      flg = 1'b1; \
                   if (!this._name[i].rand_mode()) \
                      __vmm_rhs._name[i].rand_mode(0); \
                   else __vmm_rhs._name[i].rand_mode(1); \
		end \
               if (!flg && !this._name[0].rand_mode()) \
                  __vmm_rhs._name.rand_mode(0); \
...
`define vmm_data_member_rand_handle_da(_name, _do) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
           bit flg = 0; \
...
`define vmm_data_member_rand_handle_aa_scalar(_name, _do) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
           bit flg = 0; \
...
`define vmm_data_member_rand_handle_aa_string(_name, _do) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
	   __vmm_rhs._name.delete(); \
...
`define vmm_data_member_rand_vmm_data(_name, _do, _how) \
  \
      case (do_what & _do) \
        DO_COPY: begin \
           if (_name == null) begin \
...
`define vmm_data_member_rand_vmm_data_queue(_name, _do, _how) \
     `vmm_data_member_rand_vmm_data_array(_name, _do, _how)

`define vmm_data_member_rand_vmm_data_array(_name, _do, _how) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
           bit flg = 0; \
...
`define vmm_data_member_rand_vmm_data_da(_name, _do, _how) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
           bit flg = 0; \
...
`define vmm_data_member_rand_vmm_data_aa_scalar(_name, _do, _how) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
        bit flg = 0; \
...
`define vmm_data_member_rand_vmm_data_aa_string(_name, _do, _how) \
 \
      case (do_what & _do) \
        DO_COPY: begin \
           case (_how & HOW_TO_COPY) \
...
`define vmm_data_member_end(_class) \
 ...
 \
   ...
   `ifndef DISABLE_DEF_BYTE_SIZE_IMP \
   `ifndef vmm_data_byte_size_used \
      `vmm_data_byte_size(0,0) \
   `endif \
   `endif \
   `undef vmm_data_byte_size_used \
...
`define vmm_data_methods(_class) \
 ...
 \
   virtual function vmm_data allocate(); \
      ...
      allocate = this.do_allocate(); \
      if (this.__vmm_done_user) return allocate; \
      i = new; \
      return i; \
   endfunction \
 \
...
`define vmm_data_byte_size(_max, _n) \
   `define vmm_data_byte_size_used 1 \

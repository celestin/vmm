...
//
// The notification service provides a synchronization mechanism for concurrent 
// threads or transactors. Unlike event variables, the operation of the 
// notification is defined at configuration time. Moreover, notification 
// can include status and timestamp information attached to their indication.
//
// Function: new 
// 
// Creates a new instance of this class, using the specified message service interface
// to generate an error and debug messages. 
// 
...
function void vmm_notify::set_parent_object(vmm_object parent);
   //
   // If vmm_notify has not been underpinned with vmm_object, do nothing.
   //
endfunction
`endif
...
// instance. If no instance is specified, a new instance is allocated using the same message
// service interface as the original one. A reference to the copied target instance is
// returned. 
// Only the notification configuration information is copied and merged with any pre-configured
// notification in the destination instance. Copied notification configuration replaces
// any pre-existing configuration for the same notification identifier. Status and
...
// Function: configure 
// 
// Defines a new notification associated with the specified unique identifier. If a negative
// identifier value is specified, a new, unique identifier greater than 1,000,000 is
// returned. The thread synchronization mode of a notification is defined when the notification
// is configured, and not when it is triggered or waited upon, using one of the ONE_SHOT,
// BLAST, or ON_OFF synchronization types. This definition
// timing prevents a notification from being misused by the triggering or waiting threads.
// 
// Notification Synchronization Mode Enumerated Values 
// Enumerated Value Broadcasting Operation 
// ONE_SHOT Only threads currently waiting for the notification to be indicated
// are notified. 
// BLAST All threads waiting for the notification to be indicated in the
// same timestep at the indication are notified. This mode eliminates certain types of
// race conditions. 
// ON_OFF The notification is level-sensitive. Notifications remain
// notified until explicitly reset. Threads waiting for a notification that is still
// notified will not wait. This mode eliminates certain types of race conditions. 
// 
// A warning may be generated, if a notification is configured more than once. 
// Notification identifiers numbered from 1,000,000 and above are reserved for automatically
// generated notification identifiers. Predefined notification identifiers in the
// VMM base classes use identifiers from 999,999 and below. User-defined notification
// identifiers can thus use values from 0 and above. 
// 
...
// 
// Checks whether the specified notification is currently configured or not. If this
// method returns 0, then the notification is not configured. Otherwise, it returns an
// integer value corresponding to the current ONE_SHOT, BLAST,
// or ON_OFF configuration. 
// 
...
// 
// If this method returns TRUE, then the notification is in the notify state, and any call
// to the <wait_for> method will not block. A warning is generated, if this
// method is called on any other types of notifications. 
// 
...
// Task: wait_for 
// 
// It is an error to specify an unconfigured notification. Use the <status>
...
// //////////////////////////////////////////// 
// Function: is_waited_for 
// 
// Checks whether a thread is currently waiting for the specified notification or not,
// including waiting for an ON or OFF notification to be reset. It is an error to specify
// an unconfigured notification. The function returns TRUE, if there is a thread known
// to be waiting for the specified notification. 
// Note that the knowledge about the number of threads waiting for a particular notification
// is not definitive, and may be out of date. As threads call the <wait_for()
// method, the fact that they are waiting for the notification is recorded. Once the notification
// is indicated and each thread returns from the method call, the fact that they are no longer
// waiting is also recorded. However, if the threads are externally terminated through
// the disable statement or a timeout, the fact that they are no longer waiting cannot be
// recorded. In this case, it is up to the terminated threads to report that they are no longer
// waiting, by calling the terminated> method. 
...
// notification is disabled, and is no longer waiting. 
// 
...
// Function: status 
// 
// Returns the status descriptor that is associated with the specified notification,
// when it was last indicated. It is an error to specify an unconfigured notification.
// 
// 
...
// Function: timestamp 
// 
// Returns the simulation time when the specified notification was last indicated. It
// is an error to specify an unconfigured notification. 
// 
...
`ifdef VMM_SB_DS_IN_STDLIB
   foreach (cfg._vmm_sb_ds[i]) begin
      if (cfg._vmm_sb_ds[i].is_in) begin
         if($cast(_temp_sb,cfg._vmm_sb_ds[i].sb))
            _temp_sb.insert(status);
      end
      ...
      if (cfg._vmm_sb_ds[i].is_out) begin
         case (cfg._vmm_sb_ds[i].order)
           vmm_sb_ds::IN_ORDER:
           if($cast(_temp_sb,cfg._vmm_sb_ds[i].sb))
              _temp_sb.expect_in_order(status);
           
           vmm_sb_ds::WITH_LOSSES: begin
              vmm_data p;
              vmm_data losses[];
              if($cast(_temp_sb,cfg._vmm_sb_ds[i].sb))
                 _temp_sb.expect_with_losses(status, p, losses);
           end
           
           vmm_sb_ds::OUT_ORDER:
              if($cast(_temp_sb,cfg._vmm_sb_ds[i].sb))
                 _temp_sb.expect_out_of_order(status);
...
// 
// Defines the specified notification, using the specified notification descriptor.
// If the descriptor is null, then the notification is undefined and can only be indicated
// using the <indicate> method. If a notification is already defined, the
// new definition replaces the previous definition. 
// 
...
// 
// Gets the notification descriptor associated with the specified notification, if
// any. If no notification descriptor is associated with the specified notification,
...
// Function: reset 
// 
// A <SOFT reset clears the specified ON_OFF notification, and restarts
// the vmm_notification::indicate() and vmm_notification::reset> methods on any
// attached notification descriptor. A HARD reset clears all status information
// and attached notification descriptor on the specified event, and further assumes
// that no threads are waiting for that notification. If no notification is specified,
// all notifications are reset. 
// 
//|   
...
// 
// Appends the specified callback extension to the list of registered callbacks, for
// the specified notification. All registered callback extensions are invoked, when
// the specified notification is indicated. 
// 
//|   
...
function void vmm_notify::append_callback(int                  notification_id,
   ...
   begin
      ...
      begin
         string txt;
         $sformat(txt, "Unknow notification #%0d to append callback to.",
                  notification_id);
         void'(this.log.text(txt));
         this.log.end_msg();
      end
...
// 
// Unregisters the specified callback extension from the notification service interface,
// for the specified notification. An error is generated, if the specified callback extension
// was not previously registered with the specified notification. 
// 
//|   
//|   class my_callbacks extends vmm_notify_callbacks;
//|      virtual function void indicated(vmm_data status);
...
function void vmm_notify::unregister_callback(int                  notification_id,
   ...
   begin
      ...
      begin
         string txt;
         $sformat(txt, "Unknown notification #%0d to remove callback from.",
                  notification_id);
         void'(this.log.text(txt));
         this.log.end_msg();
      end
...
function void vmm_notify::register_vmm_sb_ds(int                   notification_id,
   ...
   begin
      ...
      begin
         string txt;
         $sformat(txt, "Unknown notification #%0d to append callback to.",
                  notification_id);
         void'(this.log.text(txt));
         this.log.end_msg();
      end

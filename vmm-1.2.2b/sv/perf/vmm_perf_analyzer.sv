...
class vmm_perf_analyzer;
   ...
   protected vmm_perf_data active[$];
   protected vmm_perf_data suspended[$];
   protected vmm_perf_data completed[$];
...
function void vmm_perf_analyzer::save_db();
   foreach (this.completed[i]) begin
      ...
      void'(this.sql_table.insert(row));
   end
...
function void vmm_perf_analyzer::save_db_txt(string name = "");
   ...
   foreach (this.completed[i]) begin
      ...
      if (datum.more_data != "") begin
         $fwrite(fp, "%0d,%0d,%0d,%0d,%0d,%0d,%b,%s", tenure.get_tenure_id(),

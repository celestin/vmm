...
`endif
   ...
   `vmm_error(this.log, $psprintf("Domain \"%s\" not found in parent block %s of memory %s",
                                  domain, parent.get_name(), this.get_fullname()));
...
function void vmm_ral_mem::display_domain(string prefix = "",
                                          string domain = "");
...
function string vmm_ral_mem::psdisplay_domain(string prefix = "",
                                              string domain = "");
   string res_str = "";
   string t_str = "";
   bit with_debug_info = 1'b0;

   $sformat(psdisplay_domain, "%sMemory %s -- %0dx%0d bits @", prefix,
   ...
   foreach (this.domains[i]) begin
      if (this.domains[i] == domain) begin
         if (this.offset_in_block[i] === 'x) begin
            ...
            $sformat(psdisplay_domain, "%s'h%h\n", psdisplay_domain,
...
`else
   begin
      vmm_ral_block parent;
      $cast(parent, this._parent);
      if (inherited) return parent.get_attribute(name);
   end
`endif
...
`else
   begin
      vmm_ral_block parent;
      $cast(parent, this._parent);
      if (inherited) parent.get_all_attributes(tmp);
   end
`endif
      ...
      vmm_ral::BFM: begin
         ...
         if (this.frontdoor[di] != null) begin
            ...
            foreach (addr[i]) begin
               ...
               if (status != vmm_rw::IS_OK && status != vmm_rw::HAS_X) break;
               j += w;
               n_bits -= w * 8;
            end
         end
         ...
         if (this.cover_on) 
            parent.XsampleX(this.offset_in_block[di] + 
               mem_addr * (((this.get_n_bytes()-1)/parent.get_n_bytes(domain))+1), di, 0);
      end
...
`endif
   ...
   case (path)
      ...
      vmm_ral::BFM: begin
         ...
         if (this.cover_on) 
            parent.XsampleX(this.offset_in_block[di] + 
               mem_addr * (((this.get_n_bytes()-1)/parent.get_n_bytes(domain))+1), di, 1);
      end
      ...
      vmm_ral::BFM: begin
         ...
         if (this.cover_on) begin
            ...
            for (addr = burst.start_offset;
               ...
               parent.XsampleX(this.offset_in_block[di] + addr, di, 0);
            end
         end
      end
      ...
      vmm_ral::BFM: begin
         ...
         if (this.cover_on) begin
            ...
            for (addr = burst.start_offset;
                 addr <= burst.max_offset;
                 addr += burst.incr_offset) begin
               parent.XsampleX(this.offset_in_block[di] + addr, di, 1);
            end
         end
      end

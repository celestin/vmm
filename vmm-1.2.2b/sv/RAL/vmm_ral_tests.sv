...
class vmm_ral_tests;
   ...
   //------------------------------------------------------------------------------
   // TASK: hw_reset
   // Read every register in the specified domains of the specified block and verify that
   // the value read corresponds to the specified reset value. If the domain is specified
   // as "", all registers are exercised. Registers with an attribute named "NO_HW_RESET_TEST"
   // or "NO_RAL_TESTS" are skipped. This method must be invoked only when the block has been
   // fully powered-up and in the reset state. The state of the block must remain idle and stable
   // so the register values do no change from their initial state. 
   //------------------------------------------------------------------------------
   ...
   //------------------------------------------------------------------------------
   // TASK: bit_bash
   // Exercise every bit in the registers found in the specified domain in the specified block
   // to make sure it behaves as specified. Bits in fields with a mode specified as USER or OTHER
   // are not verified. If the domain is specified as "", all registers are exercised. Registers
   // with an attribute named "NO_BIT_BASH_TEST" or "NO_RAL_TESTS" are skipped. This method
   // must be invoked only when the block has been fully powered-up and in the reset state.
   // The state of the block must remain idle and stable so the register values do no change
   ...
   //------------------------------------------------------------------------------
   // TASK: mem_walk
   // Write then read back every location in every memories in the specified domains of the
   // specified block and verify that the value read corresponds to the previously-written
   // value. If the domain is specified as "", all memories are exercised. Memories with an
   // attribute named "NO_MEM_WALK_TEST" or "NO_RAL_TESTS" are skipped. This method must
   // be invoked only when the block has been fully powered-up. The content of the memories
   // must remain unchanged and stable so the values do no change from their previously-written
   // state.s 
   //------------------------------------------------------------------------------
   ...
   //------------------------------------------------------------------------------
   // TASK: reg_access
   // Write every register through every available front-door domain and read the corresponding
   // value through the backdoor and vice-versa. Registers with no backdoor access defined
   // or with an attribute named "NO_REG_ACCESS_TEST" or "NO_RAL_TESTS" are skipped. This
   // method must be invoked only when the block has been fully powered-up and in the reset
   // state. The state of the block must remain idle and stable so the register values do no
   ...
   //------------------------------------------------------------------------------
   // TASK: mem_access
   // Write every memory location through every available front-door domain and read the
   // corresponding value through the backdoor and vice-versa. Memories with no backdoor
   // access defined or with an attribute named "NO_MEM_ACCESS_TEST" or "NO_RAL_TESTS"
   // are skipped. This method must be invoked only when the block has been fully powered-up
   // and in the reset state. The state of the block must remain idle and stable so the memory
   // locations do no change from their initial state. 
   //------------------------------------------------------------------------------
   ...
   //------------------------------------------------------------------------------
   // TASK: shared_access
   // Write every shared register and shared memory locations through every available front-door
   // domain and read the corresponding value through every other available front door (and
   // backdoor if available) and vice-versa. Registers and memories that are not shared
   // across multiple domains or with an attribute named "NO_SHARED_ACCESS_TEST" or "NO_RAL_TESTS"
   // are skipped. This method must be invoked only when the block has been fully powered-up
   // and in the reset state. The state of the block must remain idle and stable so the register
   // values and memory locations do no change from their initial state. 
   //------------------------------------------------------------------------------
